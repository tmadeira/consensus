...
void run_path(task_t t) {
  ...
  while (1) {
    ...
    current = (B[1] % 2) * t.mem;
    r = real(engine);
    A[0] = (A[0] << 1) | (r <= old+current);
    A[0] %= 4;

    for (i = 1; i < t.n-1; i++) {
      if (2*i == t.n-1) {
        // Self-loop.
        old = ((B[i-1] >> 1) + (B[i] >> 1) + (B[i+1] >> 1)) * (1 - t.mem);
        current = ((B[i-1] % 2) + (B[i] % 2) + (B[i+1] % 2)) * t.mem;
        r = real(engine);
        A[i] = (A[i] << 1) | (3.0 * r <= old+current);
        A[i] %= 4;
        continue;
      }
    ...
    old = (B[t.n-2] >> 1) * (1 - t.mem);
    current = (B[t.n-2] % 2) * t.mem;
    r = real(engine);
    A[t.n-1] = (A[t.n-1] << 1) | (r <= old+current);
    A[t.n-1] %= 4;
